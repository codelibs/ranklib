...
public class DCGScorer extends MetricScorer {
    ...
    protected double getDCG(final int[] rel, final int topK) {
        double dcg = 0;
    ...
    //lazy caching
    protected double discount(final int index) {
        ...
        final double[] tmp = new double[cacheSize];
    ...
    protected double gain(final int rel) {
        if (rel < gain.length) {
            return gain[rel];
        }
